...
function addRecipeMetaTags(recipe) {
  ...
  const metaTags = [
    ...
    { name: 'recipe:author', content: author || 'Bakstunden Team' },
...
function addStructuredData(recipe, relatedRecipes, faqs, breadcrumbs) {
  ...
  const recipeSchema = {
    ...
    recipeCuisine: recipe.cuisine || 'Swedish',
  ...
  if (relatedRecipes.length > 0) {
    const relatedSchema = {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      name: `Relaterade ${recipe.category} recept`,
      itemListElement: relatedRecipes.map((relatedRecipe, index) => ({
        ...
        item: {
          ...
          url: `${SITE_URL}/recept/${relatedRecipe.slug}`,
